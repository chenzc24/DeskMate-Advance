...
`labeled_evidence` 软件入口已完成：`deskmate.ergonomics.annotations/1.0` 使用独立 SHA-256、精确 replay SHA、camera source 和 session-relative monotonic ns 绑定逐 lane 的 `positive/negative/transition/do_not_score` 半开区间；正例必须提供 `eligible_at_ns`。严格 parser 拒绝重复 key、非有限数字、过深/过大输入、重复 event ID、同 lane 重叠、错误来源和 replay 漂移。`replay_part_a.py run` 只有同时收到匹配的 `--annotations` 与 `--annotations-sha256` 才接受 labeled replay，并将标注 artifact 身份与正式 FPH/latency 资格写入摘要。该能力只关闭“标注文件无法进入 CLI”的软件缺口；没有真实标注数据时仍不能关闭 Gate A3。
...
当前执行状态：纯双目几何核心与合成移动不变性/质量拒绝测试已完成；真实标定、同步 pair、共同视野和移动实测均标记为 hardware-blocked TODO，不阻塞其余 Part A 软件闭环。
