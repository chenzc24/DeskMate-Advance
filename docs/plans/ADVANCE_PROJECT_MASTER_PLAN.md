...
| 屏幕过近 | P1 | 同步双目帧、人脸参考点、屏幕平面参考点 | 标定双目三角化 + 屏幕平面拟合；Face Landmarker 提供对应点 | 有效性门控、平滑、进入/退出阈值 | `screen_too_close` | 否 |
...
| 摄像头输入 | **部分冻结** | DeskMate 可搭载两台随本体移动的摄像头，屏幕距离采用移动双目候选预案；设备、刚性基线、安装视场、同步、传输、分辨率、帧率、色彩空间和时间戳仍待 robotics 冻结 | 允许准备双目标定和 recorded-pair benchmark；现有 laptop 单目结果不得用于冻结最终模型或绝对距离能力 |
...
移动双目候选的成立条件不是“摄像头数量为二”，而是两路经过内外参标定且同步的图像，对人脸参考点与屏幕平面参考点具有共同视野。双目先在同一相机坐标系重建人脸点和至少三个不共线的屏幕点，再计算人脸点到屏幕平面的垂直距离。若人脸、屏幕参考点、任一路图像、同步或标定任一证据缺失/过期，距离必须为 `unknown`；不得退化为“人到 DeskMate 的距离”并继续产生屏幕过近确认。完整预案和 Gate 见 `PART_A_ERGONOMICS_PLAN.md`。
