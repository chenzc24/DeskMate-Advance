...
RuntimeProfile
  ...
  -> SessionRuntime（一次注册、Button/筹码/清台/重买/结束）
     -> HandRuntime 1..N
       -> Part B: rotate ACK -> hole ACK default face-down / visible-card observation
...
这条路径的开发版本可运行，但不能描述为 Gate 2B/4 已通过。底牌由机构背面发出；
带完整安全与单张传感器证据的成功 ACK 直接把当前逻辑槽记为
`present_face_down`，不再要求 `F` 或人工视觉确认。机器人 Profile 使用状态机逐目标
view-cycle，YOLO 在完整画面自动框选并把无歧义稳定的正面牌绑定到当前逻辑槽；
Laptop 固定全桌 Profile 仍可保留 ROI 夹具。目标相机/物理几何与牌模型 held-out
Gate 仍未关闭。
...
绑定/确认的英语语音产生动作；`C` 确认 pending speech，Backspace 取消；底牌不需要
人工确认；`Q`/Esc 安全退出并进入暂停日志。使用 `--disable-speech` 时只做
人脸注册，不创建声纹库。
