...
- 受控原型入口：S0-01/03/04/09/10/13/22 已有明确候选、危险分析和操作员；9 个逻辑 target/命令语义保持冻结。
...
Core 优先采用固定中央旋转底座 + 单张 feeder，而不是移动机器人：四个玩家目标和五个 board 位置共 9 个目标，仍是可标定的有限角度。Core 不烧牌。feeder 可从摩擦轮/分离片方案开始，但必须用实际多副牌测试新旧程度、厚度、静电、翘曲和表面摩擦。

S0-13 要求明确比较两类 reveal：选择性翻转通道，或独立 board reveal 模块。Hole 必须 face-down，board 最终必须 face-up；若只支持人工翻牌，产品只能声明“自动分发、人工揭示”，不能声明全自动发公共牌。
...
- Hole face-down 与 board face-up 的方向控制、检测和失败恢复报告。
...
| 工作包 | 核心产物 | Gate 前证据 |
| --- | --- | --- |
| S3.0 Hazard + interfaces | 危险分析、运动包络、9 target 坐标合同、无马达协议 mock | software/MCU 互解析和错误矩阵 |
...
| S3.3 Reveal | hole 背面和 board 正面方案 | orientation、错向检测、卡堵/落点报告；或冻结 manual fallback |
